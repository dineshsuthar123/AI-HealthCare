...
// Files to exclude from validation (specialized translation files with different structures)
const excludedFiles = ['provider-en.json'];
...
function validateTranslations() {
    ...
    const files = fs.readdirSync(messagesDir)
        ...
        .filter(file => !excludedFiles.includes(file)); // Exclude specialized files
    ...
    // Check specialized files separately if needed
    console.log('\nSkipped validation for specialized translation files:');
    excludedFiles.forEach(file => console.log(`- ${file} (has a different structure)`));
